...
## Re-tidy
vancouver_trees_after <- vancouver_trees_before %>% pivot_wider(
                            names_from = diameter_and_height,
                            values_from = values) %>% 
                            unite(col = date_planted, Year, Month, Day, sep = "-", remove = T)

```

First of all, because my data is tidy and organized, so I approached to this question by first untidy the data through `pivot_longer` function by increasing number of rows and decreasing number of columns, resulting in less columns (less variables) and more rows (more observations). Therefore, there is a new column called `diameter_and_height` in which it is a column comprised of character string data types including only names of `diameter` and `height_range_id`. I also separated the `date_planted` into `Year`, `Month`, `Day` to make my
...
1.  What is the relationship between height level and tree diameters?
2.  Which height level contains the highest number of trees among which neighbourhood?
...
```{r}
vancouver_trees_2000_18_RQ2 <- vancouver_trees_2000_18 %>% 
